...
patients_raw = patient_list %>% 
  filter(disease =='Muscular_Dystrophy') %>% 
  select(z_patid) %>% 
  unlist()
...
where rs.yr>2015 AND 
rs.ndc  in ('60923028410', '60923036302','60923046502') AND rs.z_patid in (", 
...
ggsave(paste(report_directory, 'time_intervals_cost_MD.png', sep = ""),
...
p1 = ggplot(pt,
  ...
  geom_point(shape = 1, aes(color = treatment), 
             alpha = 3, position = position_jitter())+
...
p3_md = cowplot::plot_grid(p1+theme(axis.title.y = element_blank()),
                        p2+theme(axis.title.y = element_blank()),nrow = 2,
                        align = 'hv',
                        labels = c('c', 'd')
)

p3_comb = cowplot::plot_grid(p3_cf,
                   p3_md,nrow = 1,
                   align = 'hv'
)
ggsave(plot = p3_comb, filename = paste(report_directory, 'box_comb.png', sep = ""),
       dpi = 600, width = 7.2, height = 4.5, units = 'in')

write.csv(pt0, paste(report_directory,'MD.csv', sep = ''), row.names = FALSE)
ggsave(plot = p3, filename = paste(report_directory, 'MD.png', sep = ""),
...
ggsave(p1, filename = paste(report_directory, 'MD_claims_plot_year.png', sep = ""),
       dpi = 600, width = 7.2, height = 5.5, units = 'in')  
...
ggsave(p2, filename = paste(report_directory, 'MD_costs_plot_year.png', sep = ""),
       dpi = 600, width = 7.2, height = 5.5, units = 'in') 

p3 = cowplot::plot_grid(p1+theme(legend.position = 'none' ),byrow = F,
                        p2,nrow = 1,
                        labels = c('a', 'b')
)
ggsave(p3, filename = paste(report_directory, 'MD_comb_plot_year.png', sep = ""),
       dpi = 600, width = 10, height = 3, units = 'in') 

p3_md_dist = cowplot::plot_grid(
  p1+theme(legend.position = 'none',
           plot.title = element_blank()),byrow = F,
...
dist_comb = cowplot::plot_grid(
  p3_cf_dist,
  p3_md_dist,
  byrow = F,nrow = 2
)
ggsave(dist_comb, filename = paste(report_directory, 'dist_comb_year.png', sep = ""),
